...
data%>%
  mutate(Trend=fct_relevel(Trend,"Indian Premier League","CoWIN","ICC T20 World Cup","Euro Cup","Tokyo Olympics","COVID Vaccine",
                           "Free Fire Redeem Code","Copa America","Neeraj Chopra","Aryan Khan"))->data1

ggplot(data1,aes(x=Week,y=Value,fill=Trend))+
  geom_area()+
  geom_line(colour="white")+
  facet_wrap(~Trend,nrow=10)+
  ...
  labs(title="WHAT WAS TRENDING IN INDIA IN 2021?",
       subtitle = str_wrap("The below visualization shows the trend interest over time in 2021, on Google, for the top 10 topics that were searched the most, on web",108),
...
ggsave("searchtrends.png",plot,width = 10,height=14)
